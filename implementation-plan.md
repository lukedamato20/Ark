...
- **Status: Blocked by genuine external dependency (2026-08-14).** `config/release-capabilities.json` is the versioned authority for candidate OS/runner support, minimum window size, provider visibility, delivery mode, model expectations, routing/privacy class, and unavailable capabilities. `pnpm support:check` validates that configuration against Tauri window settings, the CI matrix, provider identifiers, UI gates, and documentation; Settings and README now truthfully distinguish externally installed runtimes from bundled capability. The built-in path is disabled until its setup installs a verified binary. `docs/support-matrix.md` is linked from user documentation and explicitly labels the matrix as a candidate. The only unmet criterion is engineering/product review: no authorized product owner is available in this environment. Unblock by reviewing the candidate matrix and its platform evidence; on approval, change `reviewStatus` to `approved` and record the approvers/date in the support matrix.
...
- **Status: Blocked by genuine external dependency (2026-08-14).** `docs/adr/0001-durable-generation-lifecycle.md` defines queued/starting/streaming/cancelling/complete/interrupted/failed/cancelled semantics, transaction and event boundaries, idempotent terminals, cancellation/retry/edit/regenerate/import behavior, and all required crash points; COR-001–005 now conform to it and their executable tests exercise the contract. The remaining acceptance criterion is formal engineering/product approval, and its required timing (“approved before COR-001–005 implementation”) was already missed by the prior implementation sequence and cannot be retroactively claimed. Unblock requires authorized reviewers to approve the ADR and explicitly decide whether to accept that historical deviation or amend the plan; this agent cannot self-approve or weaken that criterion.
