...
- **Status: Partial — projects are complete and real; personas/prompt library are not started (2026-08-15).** Investigated the actual state before writing anything: unlike several other Phase 5 items this session, this one was genuinely greenfield — no `projects` table/migration existed, `conversations.project_id` was only a nullable filter seam with nothing to reference (added by `0004_scalable_history_search.sql`, not by ARC-007 as an earlier note in this plan assumed), no persona/prompt concept existed anywhere in the codebase (`grep -ri persona` hit only docs), and `generation.rs`'s precedence chain was three-tier (request → conversation → provider), explicitly not yet including a project tier. Given the size of this item — projects and personas are two largely independent capabilities, and building both fully in one pass risked a half-finished mega-change — this pass scoped to projects only, executed to completion with the same rigor as every other item this session, and stopped there rather than jam personas in partially done.
  ...
  - **`generation.rs`'s precedence extended from three tiers to four**: `resolve_setting` now takes `(request, conversation, project, provider)` for temperature/max_tokens, and a new `resolve_system_prompt(conversation, project)` handles the system prompt (which has no per-request tier). `SettingSource` gained a `Project` variant. `GenerationProvenance` gained `project_id` and replaced the old boolean `system_prompt_applied` with `system_prompt_source: Option<SettingSource>`, so provenance now records *which* tier actually supplied each value, not just whether the conversation had an override — the first half of "precedence... is documented and visible" (criterion 1): the chain itself is now real and inspectable per-generation, though a user-facing precedence explainer in the UI was not built (see "Not done," below). A `resolve_conversation_project` helper treats a conversation whose `project_id` no longer resolves as unassigned rather than failing the send — defensive against a state that shouldn't be reachable (deletion always unassigns first) but costs nothing to guard.
  - **Frontend**: a new "Projects" panel in Settings (`ProjectsPanel`/`ProjectEditor` in `SettingsView.tsx`) — create, list (active-first, then archived), edit every field, archive/unarchive, and delete with the preview surfaced before the user confirms ("N conversation(s) will be unassigned, not deleted"). A project picker was added to `ChatView`'s existing `ConversationSettingsButton` panel so a conversation can be assigned/reassigned/unassigned from a project inline, next to its own system-prompt/temperature/max-tokens override controls.
  - **A real bug found and fixed during live verification, not just typechecking**: the project editor's temperature/max-tokens fields initially reused `NumberField`/`validateNumberInput`, which are built for `ProviderForm`'s *always-required* defaults and treat an empty field as an error ("Temperature is required"). For a project default, empty is a legitimate, meaningful state ("no project override, inherit the provider's"), the same shape `ConversationSettingsButton`'s own temperature/max-tokens fields already handle correctly with bespoke inline validation. Reused that existing pattern instead — plain `Input` fields with custom empty-is-valid logic — rather than the shared component built for a different case. Caught this live in the browser (the Save button was wrongly disabled with a false "required" error on an intentionally-empty field), not by reading the code.
  - **Live-verified in the browser**, via a new stateful project fixture (extended `createConversationOrganizationFixtureClient`, `?fixture=conversation-organization`, one seeded "Research" project pre-assigned to a conversation): created a project, edited every field including clearing temperature/max-tokens back to empty and confirming that persisted correctly (the bug above, confirmed fixed), previewed a deletion and saw the correct affected-conversation count, confirmed the delete and saw the project disappear from the list, created a fresh project via the inline form, and reassigned/unassigned a conversation's project via `ChatView`'s picker — all round-tripping through the fake client with zero console errors (one transient HMR-reload error from mid-edit, confirmed stale by a subsequent fresh reload showing none, not a real bug).
  - **Not done, honestly — this is a Partial status, not Complete:**
    - **Personas/prompt library (criterion 2) — not started at all.** No table, no versioning, no UI. This is the other half of the plan's Description and a full acceptance criterion on its own; it needs its own migration, its own precedence tier (persona sits between conversation and project per the plan's stated order), and its own library UI, which didn't fit in this pass without risking a rushed, under-tested implementation.
    - **Precedence documentation/visibility (criterion 1) is only half-done.** The precedence chain itself is real and now four-tier, and every generation records which tier won in provenance — but there's no user-facing explainer surfacing "application → project → persona → conversation → user" as a documented, visible concept anywhere in the UI, partly because "application" and "persona" tiers don't exist yet to document.
    - **Instructions/untrusted-content separation (criterion 4) is unaddressed, and structurally can't be meaningfully attempted yet**: `ProviderChatRequest`/`ChatMessage` are unchanged from FTR-004's shape (`{ role, content }`), with no separate channel for injected instructions vs. retrieved content. This is because there is still no file/attachment/retrieval feature anywhere in Ark producing untrusted content to separate *from* — building a separation mechanism now would be speculative scope with nothing real to protect against yet, not a gap in what this task set out to do.
    - **Files/context policy** (part of the Description) — no code anywhere; out of scope for this pass, tracked as remaining project scope.
...
- **Status: Partial — naming and comparison are new and real; a dedicated tree/topology explorer view was not built (2026-08-15).** Investigated the actual state before writing anything: unlike a full "explorer," a real per-message alternatives switcher already existed (`ChatMessageList.tsx`'s "Branches" toggle) — content previews, active/status badges, and a "+ history" descendant-impact hint were already there, all working. What genuinely didn't exist: any way to name a branch, and any way to see two alternatives at once instead of switching back and forth. Those are the two acceptance criteria this pass actually closes; "switch explicitly" and "descendant impact before switching" were already true going in, not new work from this task.
  ...
  - **Not done, honestly — this remains Partial, not Complete:**
    - **No dedicated "branch explorer" view.** The plan's own suggested implementation notes call for "a compact branch indicator in chat and a dedicated explorer/drawer for full topology" — this pass extended the existing compact indicator (naming, comparison) but did not build the second half: a standalone view showing the *entire* conversation's branch tree at once (every divergence point, not just the one the user currently has expanded). For a conversation with multiple branch points, there's still no single place to see the whole shape of the tree.
    - **"Branch names... survive restart/sync"** — restart is untrue-to-verify without a real database in this pass (covered by the migration/DB-layer tests, which do prove persistence across reopen at the SQL level) but not re-confirmed via an actual app restart in the browser. "Sync" has no meaning yet — no sync feature exists anywhere in Ark.
    - **Comparison provenance is the existing lightweight fields, not a parsed view of `GenerationProvenance`** (temperature/max-tokens/system-prompt source, project id) — that richer provenance is written to `metadata_json` by `generation.rs` (FTR-004) but nothing in the frontend parses it anywhere, comparison view included. Extending the comparison view to show it would be real, scoped follow-up work.
...
- **Status: Partial — the Ollama-specific acceptance criteria are now genuinely complete; the built-in-runtime model pipeline (the item's Extra Large half) was not attempted (2026-08-15).** Investigated before writing anything: browsing installed models and pulling-with-progress already existed (`OllamaProvider::list_models`/`pull_model` via `/api/tags`/`/api/pull`, wired through `provider_management.rs` and a working `OllamaModelsPanel` in Settings) — contrary to the plan's Reason field implying nothing existed yet. What was genuinely missing, verified by reading every relevant call site: pull cancellation (no mechanism existed at all), the delete confirmation didn't state disk footprint, and `/api/tags`'s `details` object (family/parameter_size/quantization_level) was already being fetched and stored in `metadata_json` server-side but never parsed or shown by the frontend. Separately, there is still no download/verify/catalog pipeline anywhere for the *built-in* llama.cpp runtime's own models — that remains exactly the "incomplete manual scaffold" the plan describes, untouched by this pass. Given the size difference between "finish the already-mostly-built Ollama UX" and "build an entire verified-download subsystem from scratch," this pass scoped to the former.
  ...
  - **Not done, honestly — this remains Partial, not Complete:**
    - **No built-in-runtime model download pipeline at all** — the plan's own largest piece: discovery/catalog, verified/resumable/atomic download, storage selection, hardware-fit warnings before download/load. `BuiltInRuntimeForm` still takes a free-text local file path the user must already have downloaded themselves plus manually-typed provenance strings; nothing changed here. This is genuinely Extra-Large-scope greenfield work, not a gap this pass could close alongside the Ollama fixes without risking a rushed implementation of the highest-risk part of the item (verified download integrity).
    - **Ollama library browsing** ("models available in the Ollama library... to pull," not just already-installed ones) — not attempted. The plan's own suggested notes flag this as dependent on an external registry API of uncertain stability; installed-model browsing (already real) and pull-by-known-name (already real) were judged the higher-value, lower-risk pair to finish first.
    - **Context length** per model — Ollama's `/api/tags` (what `list_models` calls) doesn't return it; only `/api/show` does, which nothing in Ark calls. Not added in this pass.
    - **Runtime update, load/unload for the built-in runtime, storage selection** — all part of the untouched built-in-runtime scope above.
...
- **Status: Partial — the stored-credential-to-request wiring gap is fixed; everything else in this large item is untouched (2026-08-15).** Investigated before writing anything: SEC-001 (destination classification/risk acknowledgement) and SEC-005 (OS-keychain credential storage, `upsert_provider_secret`/`get_provider_secret_metadata`/`delete_provider_secret`, frontend credential UI) were both already fully built — but a real gap remained between them. `ProviderConfig.api_key_ref` was stored and the frontend already let a user save a credential against a remote OpenAI-compatible endpoint (the existing `local_inference_host` provider type, generic OpenAI-compatible chat completions), but nothing ever read that stored credential back out and attached it to the provider's actual outgoing requests — only the built-in runtime's own sidecar-generated token got an `Authorization` header. A saved API key for a real cloud endpoint was silently inert. Also confirmed there is currently no way to create or delete a provider at all — only the three seeded ones (`built_in`, `ollama`, `local_inference_host`) exist, ever updated in place, never added or removed — which makes criterion 5 ("provider deletion revokes secrets") currently unreachable rather than failing, and means "add a named cloud provider" isn't yet a real user flow independent of reconfiguring `local_inference_host`'s base URL.
...
- **Status: Partial — workspace/project-scoped batch export and import with content-hash duplicate detection are new and real; semantic merge and attachments were not attempted (2026-08-15).** Investigated before writing anything: only per-conversation JSON/Markdown export and single-conversation JSON import existed (COR-009) — there was no way to export or import more than one conversation at a time, and no manifest of any kind. Batch/project/workspace scope, versioned manifests, and a real duplicate-detection signal were the genuinely missing pieces this pass closes.
...
- **Status: Complete for three of four acceptance criteria, Partial on the fourth (2026-08-14).** Investigated each criterion against the actual current behavior before writing anything (not assumed from the plan's "Reason" field) — all four turned out genuinely unmet, unlike several other Phase 5 items this session where the underlying problem had already been fixed by earlier work.
...
- **Status: Partial — a real, disabled-by-default, loopback-only, authenticated, versioned API now exists with two read-only endpoints; paired-LAN mode, write endpoints, and OpenAPI publication were not attempted (2026-08-15).** Investigated before writing anything: nothing existed yet at all — SEC-010 (a prerequisite, already Complete) had only defined the threat model this task builds against. Followed that threat model directly rather than inventing a new one: custom-header bearer auth with zero exempt routes (not even health), never a cookie, reusing SEC-002's already-proven loopback-listener/auth-proxy pattern (`proxy.rs`) as the structural template for the new `companion_api.rs`.
