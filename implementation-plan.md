...
- an optional, provider-agnostic Ark Code agentic coding environment built on the tool/agent foundation, addressable independently of Ark Chat and used through the same conversation-first coding-agent interaction model users expect from Claude Code and OpenAI Codex;
...
| 6.5 — Ark Code (agentic coding environment) | Deliver a provider-agnostic, local-model-first agentic coding assistant — repository awareness, scoped file/git/command tools, a durable agent loop, approvals, and a conversation-first Claude Code/Codex-equivalent user workflow — as a distinct application surface alongside Ark Chat | CODE-001–008 | Phase 6 tool/agent and security foundation (CMP-003, SEC-009, ARC-003); Phase 5 desktop feature-complete Ark Chat | Persistent coding conversations drive autonomous read/edit/test loops; editing/execution tiers remain capability- and approval-gated |
...
| Ark Code | Full-stack + Desktop/product + Platform/security engineer | Tool-calling protocol, repository-tool design, and conversation-first interaction design after ARC-003, CMP-003, SEC-009 land | Owns the agent-run lifecycle, coding-conversation projection, and tool-execution contract; must not alter the chat generation lifecycle (FND-002) or duplicate the chat message schema, provider registry, tool permissions, or design primitives |
...
#### Ark Code product and interaction contract

Ark Code is separate from Ark Chat in authority, lifecycle, persistence, and available tools, but it is **not** a dashboard of manual repository utilities. The normal user workflow must be identical in interaction model to Claude Code and OpenAI Codex: the user opens a persistent coding conversation, describes the desired outcome in natural language, and collaborates with an agent that investigates, edits, runs approved commands, reports progress, requests approval when required, and returns a verifiable result in one chronological thread.

"Identical" here means equivalent user workflow, mental model, control placement, information hierarchy, and feedback behavior. It does not mean copying another product's trademarks, proprietary assets, wording, or exact pixels. Ark retains its own visual identity, local-first/provider-agnostic architecture, explicit privacy routing, accessibility standards, and stricter capability/security boundaries. If Claude Code and Codex differ, Ark follows the shared interaction pattern below and chooses the safer, clearer behavior.

The following requirements are authoritative for every CODE task and supersede any earlier "minimal shell," step-by-step dashboard, or manual-tool-panel interpretation:

- **First-class mode switch:** Ark's primary navigation lets the user switch predictably between Ark Chat and Ark Code. They are sibling application surfaces, not mixed in one transcript, and switching never loses either surface's active state.
- **Conversation is the primary surface:** Each Ark Code session is a persistent coding conversation bound to one Project and Repository. The transcript, not repository/tool forms, occupies the primary content area.
- **Composer-driven use:** A persistent composer accepts the initial request and natural-language follow-ups. Provider/model, repository, permission mode, and context controls are visible but subordinate to composing the request. The user never has to translate intent into a tool form.
- **Automatic controlled agent loop:** Sending a request starts or continues the durable agent loop automatically. The executor continues across model/tool/observation steps until it completes, needs approval or clarification, reaches a budget/safety boundary, is cancelled, or fails. A normal user must never click "Run step" for each model turn; single-step execution may exist only behind an explicit developer/debug surface and is not release UX.
- **One chronological, typed timeline:** User messages, concise agent progress updates, assistant responses, plans, tool calls, bounded tool results, command output, file changes, diffs, approval requests, context compaction, warnings, errors, recovery decisions, and final summaries appear in causal order in the conversation. Details are progressively disclosed without making activity invisible. Ark shows useful progress summaries, not private chain-of-thought.
- **Inline review and approval:** Model-proposed edits, commands, network access, and other side effects appear at the point they are requested. The user can inspect the exact scope/diff/command, approve, reject, or provide a revised instruction without leaving the conversation. Approval remains narrow, precondition-bound, expiring, and auditable under ADR 0002/0003.
- **Direct user control:** While work is active the user can stop/cancel safely, answer a clarification, approve/reject a side effect, or add a steering instruction according to the lifecycle contract. After completion/interruption/failure the same composer supports follow-up, continue, retry, or an explicit child-run attempt without starting over.
- **Supporting panes are secondary:** Session navigation, changed files, diff review, repository tree/search, terminal/output, context/budget details, and diagnostics may use side panes or drawers. They support the conversation and must not replace it. Standalone manual repository/edit forms are development harnesses, not the production interaction model.
- **Live, durable feedback:** Text and progress stream incrementally where the provider permits; tool state and approvals update without refresh; closing, crashing, or reopening reconciles from durable state. The transcript always distinguishes active, waiting, stopped, interrupted, failed, and completed work.
- **Provider-independent degradation:** Native and prompted tool-calling models use the same user workflow. Capability differences change what the agent can do and what Ark explains, not the overall layout or interaction model. Local models remain first-class; small context windows compact visibly rather than silently losing state.
- **Accessible and responsive:** The complete coding conversation, composer, tool cards, diffs, approvals, terminal output, keyboard flow, focus restoration, live announcements, zoom/reflow, reduced motion, and supported viewport states meet the same UX-001–009 and WCAG 2.2 AA gates as Ark Chat.

Behavioral parity must be maintained as an explicit, reviewed acceptance artifact: CODE-007 records the current Claude Code/OpenAI Codex comparison checklist used for implementation and release verification. A later upstream UI change does not silently redefine Ark; adopting it requires a reviewed plan update that preserves Ark's principles and security model.
...
- **Product-direction correction (2026-08-17):** The standalone Edit File panel remains valid as a development harness for the backend safety contract, but it does not satisfy CODE-005's production interaction requirement. Completion requires model-proposed edits and all later write/command operations to enter CODE-007's chronological conversation as inline, precondition-bound review items.
- **Conversation-integration continuation (2026-08-17):** The production agent loop now advertises `edit_file` as a proposal-only provider tool. A valid model call is strictly decoded, previewed against current Repository state, and atomically committed as a `proposed` invocation with its exact diff and approval hashes while the run pauses in `awaiting_approval`; no write or synthetic tool result occurs. `CodeView` renders the diff inline at its causal point with Approve/Reject actions. Approval echoes the displayed call/preview/precondition hashes, re-previews current state, durably records `executing_tool` intent plus a leased file-hash verification plan before filesystem I/O, performs the existing atomic write, commits verification evidence, and resumes the automatic agent loop only after an `applied` observation. Rejection durably records `denied`, a tool-error observation, and an interrupted run so a revised natural-language child run remains explicit. Cancellation races, duplicate/stale decisions, executor ownership, and startup recovery are fail-closed: uncertain edits are hash-classified as applied/not-applied/diverged/unknown after lease expiry and are never replayed. Focused model-wire and SQLite lifecycle tests prove proposals do not mutate, stale approvals roll back, and only an owned verified execution continues. CODE-005 remains **Partially complete** because dedicated Git checkpoint/rollback and user-configured allowlisted command execution (including their inline approval cards) are still unimplemented.
- **Status: Complete (2026-08-17, final continuation).** Ark Code now provisions one managed local clone per session under Ark's workspace and performs all agent work on `ark/session/<session-id>`, so the user's checkout, dirty tree, index, untracked files, and active branch are never touched. A non-Git Project receives an explicit `git init` offer and Ark refuses to invent the first user commit. `git_checkpoint` previews the exact status/tree diff, creates an approval-bound commit with `commit-tree` plus atomic `update-ref`, records its commit/tree/parent as an Ark-owned checkpoint, and verifies the result. `git_rollback` accepts only a durable checkpoint ID, previews the removed history/diff, and refuses any history containing a commit Ark did not record before resetting and cleaning only the isolated clone. `run_verification_command` accepts only the ID of an enabled local-user-defined build/test/lint template; shells, scripts, paths, Git/network utilities, model-supplied arguments/cwd/environment/timeouts, and disabled/substituted IDs are rejected. Execution uses the isolated Repository as cwd, a stripped environment, bounded output/time, direct argument vectors, and a cancellation handle that kills the child. All three operations share the existing durable proposal/approval/audit lifecycle, appear as typed inline conversation cards, revalidate call/preview/precondition hashes immediately before execution, verify actual state afterward, resume only on an observed outcome, and recover fail-closed without replay after a crash. Migrations `0020_ark_code_execution_policy.sql` and `0021_code_agent_streaming_text.sql`, typed Rust/TypeScript client contracts, Settings allowlist management, startup recovery, and focused isolation/proposal/approval/cancellation/rollback tests complete every acceptance criterion.
...
- **Acceptance criteria:**
  ...
  - Model-proposed edits, checkpoints, rollbacks, and commands are presented as typed inline timeline items in the Ark Code conversation; the user can inspect, approve, reject, or respond with a revised instruction without navigating to a standalone tool form.
...
#### CODE-006 — Implement coding-conversation context and token-budget management

- **Description:** Add a real per-model context-window budget allocator for Ark Code conversations (system/tool instructions, user/assistant turns, plan, sliding window of tool observations, open file contents), with explicit, visible compaction (superseded reads collapsed to summaries) instead of silent truncation. Follow-up messages continue the same durable conversation and preserve the causal relationship between user turns and child runs.
- **Status: Complete (2026-08-17).** Ark Code now refuses to guess when a model has no usable reported `contextWindow` and allocates every provider step against the selected model's actual window plus the run's remaining token budget. A deterministic tokenizer-independent estimator accounts separately for system instructions, serialized tool schemas, user/assistant turns, untrusted tool/file observations, provider-envelope overhead, a five-percent safety margin, and a context-scaled response allowance (one eighth of the model window, bounded to 256–2,048 tokens), so small local models produce shorter planning turns instead of inheriting a cloud-sized constant. Context follows only the current run's immutable parent chain—sibling retries are alternate histories and never mix—preserves the current user turn, selects a newest-first suffix of earlier turns and repository evidence, and records estimated input/output/reservation counts in the durable pre-dispatch prompt manifest. When older turns or observations do not fit, the same bounded compaction summary is both supplied to the model as untrusted data and committed atomically as a `system` observation plus sequenced `context_compacted` event; `CodeView` renders it as a distinct, live-announced timeline item rather than silently truncating. Pure allocator tests cover small-window compaction/current-turn/newest-evidence preservation and explicit required-prompt failure; a real provider-wire integration test proves a child run receives its causal parent user/assistant conversation; SQLite ownership tests prove the compaction observation/event is atomic with step reservation. Validation: Rust formatting and strict all-target/all-feature Clippy pass; 524 Rust tests pass with one existing ignored and exactly the same two locked Windows credential-store tests filtered after their environment failure was reconfirmed; TypeScript, ESLint, Prettier, 67 frontend tests, production Vite build, 84-type contract, module-boundary, and design-token checks pass.
...
- **Acceptance criteria:**
  ...
  - Initial requests and natural-language follow-ups share one durable coding conversation; continuation/retry creates the lifecycle records ADR 0003 requires without making the user start a disconnected session.
  - Eviction/compaction is visible as a typed item in the conversation timeline, not silent.
...
- **Description:** Add a third `ActiveView` alongside `chat`/`settings`, its own frontend feature module, domain store, and `ArkClient` methods/events, plus the `code_sessions`/session-event schema (through a versioned migration) needed to persist and resume agent runs. Implement the authoritative Ark Code product and interaction contract above: a conversation-first coding workspace with a persistent composer, automatic controlled agent loop, typed chronological activity timeline, inline diffs/approvals, natural-language follow-ups, and supporting repository/diff/terminal panes.
- **Status: Partially complete (2026-08-17) — the conversational MVP and inline edit approvals now exist; streaming and the broader write/tool surface remain.** Ark has a first-class `code` ActiveView, normalized domain store, persistent Project-owned sessions/runs, hard budget and Repository-identity checks, automatic provider/tool execution, durable cancellation/recovery, and a chronological conversation with a persistent composer. Read-only Repository calls execute directly; `edit_file` calls become inline approval-gated proposals and only resume after hash-bound execution verifies as applied. Authoritative SQLite state and versioned refetch notifications reconcile missed UI events and startup recovery; abandoned edits are classified from actual file hashes after lease expiry and never replayed. A third consecutive identical tool call is stopped before execution and durably fails with an explicit loop reason. Direct/single-step commands remain deterministic development seams rather than the production interaction. CODE-007 remains incomplete: incremental assistant-text streaming; Git checkpoint/rollback and allowlisted command integration; clarification/active steering; deterministic retry/revision controls; secondary Repository/diff/terminal panes; focused component and packaged E2E coverage; and the reviewed Claude Code/Codex behavioral-parity artifact.
- **Executor durability continuation (2026-08-17):** Normal execution now implements ADR-0003's pre-dispatch boundary rather than merely changing a run state: a single `BEGIN IMMEDIATE` transaction claims `planning`, creates a `reserved` step with its prompt manifest, reserves the conservative token budget, records a renewable 30-second executor lease, and appends the sequenced event before provider I/O. A separate ownership-checked transaction marks the step `dispatched`; provider and read-only tool waits renew the lease every 10 seconds; completion, cancellation, and error terminalization require the same lease; an ownership loser stops without interrupting the winning process. Successful intermediate steps retain that process's lease across `observing`; startup performs an immediate recovery sweep plus a delayed post-expiry sweep, releases a never-dispatched reservation, and conservatively retains a dispatched request's reservation when usage is unknowable. Focused SQLite tests prove duplicate-claim exclusion, foreign renewal/commit rollback, owner-only terminalization, pre/post-dispatch recovery, and reservation handling. Rust formatting, strict all-target/all-feature Clippy, 40 Ark Code-focused tests, and the full repeat excluding exactly the two locked Windows credential-store tests pass (521 passed, one existing ignored, two filtered); the unfiltered run confirms only those same environment failures. TypeScript, ESLint, Prettier, 67 frontend tests, production Vite build, 84-type contract, module-boundary, design-token, secret-boundary, CSP, Markdown-safety, support-matrix, reference-dataset, and supply-chain checks pass.
- **Inline edit-approval continuation (2026-08-17):** CODE-005's model-proposed `edit_file` lifecycle is now integrated into this conversation surface: typed proposal/diff cards pause the durable run, exact hash-bound Approve/Reject decisions update the authoritative timeline, a verified approval resumes the same automatic loop, and startup reconciles an abandoned executing edit from actual file hashes without replaying it. The process-local executor releases ownership before publishing a pause, preventing an immediate approval from being mistaken for a duplicate executor. ADR-0003's runaway guard is also enforced before execution: a third consecutive identical tool call is persisted as a tool error and fails the run with an explicit reason instead of spending another operation/budget cycle. Remaining CODE-007 work is incremental assistant-text streaming, clarification/active steering, deterministic retry/revision controls beyond the existing child-run follow-up, secondary repository/diff/terminal panes, focused component and packaged E2E coverage, and the reviewed Claude Code/Codex behavioral-parity artifact. Git checkpoint/rollback and allowlisted command cards remain gated by CODE-005.
- **Production implementation continuation (2026-08-17):** Durable assistant text now checkpoints during provider streaming and renders before terminal completion; committed observations replace the transient draft without duplicating text. The same composer supports active Stop & steer, typed clarification answers, deterministic child-run continuation/retry, and revision after a rejected proposal. Exact inline edit/checkpoint/rollback/command cards, progress/tool/error/compaction/recovery/final states, and optional Repository tree/search, changed-files/diff, command-output, context/budget, and lifecycle-diagnostics panes all remain in one chronological conversation. Ark Code restores the selected session plus composer draft/focus and timeline scroll across Ark Chat/Ark Code switches. Long histories mount a bounded 20-run causal suffix with accessible incremental loading and stable prepend anchoring. `docs/ark-code-behavioral-parity-v1.md` records the then-current first-party Claude Code/Codex comparison and Ark's stricter intentional local-security differences. The production implementation and deterministic Rust/frontend coverage are complete, but CODE-007 remains **Partially complete** because its own acceptance criteria require (1) product approval of that artifact, currently pending, (2) rendered DOM/axe/viewport component coverage whose shared TST-004 harness is separately blocked on the repository-wide component-test infrastructure decision and unavailable packages, and (3) installed-binary workflow automation whose TST-005 native driver/platform prerequisites remain a genuine external dependency. These gates are not replaced by the passing production build, 71 DOM-free frontend tests, or deterministic backend integration fixtures.
- **Reason:** Ark Code must be immediately understandable and usable as the same conversation-driven coding-agent workflow users know from Claude Code and OpenAI Codex, while remaining a distinct Ark surface and reusing Ark's existing container/store/typed-client, provider, lifecycle, permission, and design-system architecture.
- **Related audit findings:** None (see phase note).
- **Dependencies:** ARC-002, ARC-008, ARC-005, UX-001, UX-003, UX-004, UX-006–009, CODE-002–006. Staged conversation-shell work may begin against CODE-004's read-only tier, but this task cannot be Complete until CODE-005/006 are integrated into the same interaction model.
- **Priority / complexity:** High / Extra Large.
- **Expected outcome:** A user switches from Ark Chat to Ark Code, opens or resumes a coding conversation, describes an outcome, and collaborates through natural-language turns while the agent automatically investigates, proposes and applies approved changes, runs allowed verification, and reports a grounded result without requiring the user to operate individual tools or model steps.
- **Acceptance criteria:**
  ...
  - Primary navigation provides an accessible, persistent Ark Chat/Ark Code switch; each surface preserves its own selected session, scroll/focus state, and active work without mixing chat messages or repository-execution authority.
  - The primary Ark Code surface is a persistent chronological coding conversation with session history and a composer available for both the initial request and natural-language follow-ups. A dashboard, manual Repository inspector, or Edit File form cannot be the primary release interaction.
  - Sending a request starts/continues the durable executor automatically and advances model/tool/observation steps until completion, approval/clarification, cancellation, budget/safety stop, or failure. Per-step "Run step" interaction is absent from normal production UX.
  - Streaming assistant text and concise progress, plans, tool calls/results, command output, file changes, diffs, approvals, compaction notices, errors, recovery decisions, and final summaries render as causally ordered typed timeline items with bounded, progressively disclosed details; private chain-of-thought is never exposed or requested.
  - While work is active the user can stop/cancel safely and respond to clarification or approval requests; terminal/interrupted/failed work offers continue, retry, or explicit child-run actions through the same composer with deterministic focus behavior.
  - Model-proposed edits and commands produce inline, exact, precondition-bound review cards. Approve, reject, and revised-instruction paths are complete, keyboard accessible, auditable, and preserve ADR 0002/0003 semantics.
  - Session list, composer, tool-execution cards, diff cards, approval prompts, terminal/output surfaces, state panels, menus, and dialogs reuse Ark's existing UI primitives/tokens and pass responsive, keyboard, screen-reader, zoom/reflow, contrast, reduced-motion, and live-announcement gates.
  - Repository tree/search, changed-files/diff, terminal/output, context/budget, and diagnostics views are optional supporting panes/drawers; closing them never blocks the conversational workflow, and they never become alternate ungoverned tool paths.
  - Native and prompted tool-calling providers, including supported local models, present the same interaction model. Missing capabilities and small context limits degrade explicitly in the timeline rather than changing Ark Code into a different UI or silently truncating context.
  ...
  - Focused component and packaged E2E tests cover new session, follow-up, automatic multi-step execution, streaming, tool result, approval/rejection/revision, cancellation, completion, failure, budget stop, compaction, close/reopen recovery, and Ark Chat/Ark Code switching.
  - A versioned behavioral-parity checklist compares the implemented workflow with the then-current Claude Code and OpenAI Codex interaction patterns, records intentional Ark security/privacy/accessibility differences, and is reviewed by product before CODE-007 is marked Complete.
- **Potential risks:** Mistaking visual imitation for interaction parity; exposing private reasoning; approval fatigue; a UI that appears autonomous while lifecycle recovery is incomplete; allowing provider differences to fragment the user workflow; scope growth into unrelated upstream features.
- **Suggested implementation notes:** Replace the current primary dashboard incrementally: first project the existing durable read-only steps into the conversation timeline and add the persistent composer, then background/stream the executor and add stop/recovery, then integrate CODE-005 diffs/approvals and CODE-006 compaction. Keep manual tool forms only as a clearly separated development fixture until removed or intentionally retained outside production builds.
...
- **Status: Complete (2026-08-17).** The checked-in `tests/fixtures/ark_code_adversarial/injected.rs` fixture combines instruction override, secret-exfiltration, destructive-command, trusted-state spoofing, and approve-all attempts. A real two-step provider/tool integration test proves hostile file content remains serialized only in Ark's labeled untrusted-context message and never enters system instructions. Provider-dispatch tests traverse every model-supplied path-bearing read tool; edit traversal, ignored/symlink/binary/oversized file, bounded listing/search/diff, external Git metadata, and strict schema tests cover the remaining repository surface. Git regression tests prove the managed clone excludes dirty/untracked user work, uses a dedicated branch, leaves the source checkout unchanged through checkpoints/rollback, and refuses rollback across an unrecorded commit. Command tests reject shells/scripts/paths/network/Git tools, oversized arguments, disabled/substituted IDs, verify stripped fixed execution, and prove cancellation is killable. Exact approval-hash, duplicate/stale-decision, audit-event, no-auto-replay recovery, and three-identical-call loop tests cover bypass/fatigue patterns. Frontend timeline classification derives trusted clarification/approval/tool presentation only from durable tool identity/state—never hostile content—and every repeated proposal remains a distinct invocation requiring its own approval. These tests run through the existing Rust and frontend commands already required by `.github/workflows/ci.yml` on every pull request.
...
- **Acceptance criteria:**
  ...
  - Conversation-timeline tests prove hostile model/tool text cannot spoof trusted progress, approval, diff, command, completion, or system-state items, and repeated prompts cannot create an effective "approve all" interaction.
...
- **Description:** Combine paginated/indexed queries with conversation/message virtualization or windowing, stable scroll anchoring, and bounded branch data loading for Ark Chat and Ark Code's mixed text/tool/diff/terminal activity timeline.
...
- **Acceptance criteria:**
  ...
  - A long Ark Code fixture with interleaved progress, tool results, command output, diffs, approvals, compaction notices, and assistant text remains responsive without losing causal order, keyboard focus, or accessible status.
...
- **Acceptance criteria:**
  ...
  - Ark Code's conversation/composer, automatic-run progress, tool/result, diff, approval, clarification, cancellation, compaction, recovery, and completion states have semantic keyboard and accessibility tests; the responsive suite proves supporting panes collapse without replacing or obscuring the primary timeline.
...
- **Description:** Automate clean launch, onboarding, provider setup, chat/stream/stop/retry/restart, Ark Chat/Ark Code switching, Ark Code conversation/follow-up/automatic execution/approval/recovery, branch, search, settings, import/export, backup/restore, model lifecycle, updates, and critical mobile flows.
...
- **Acceptance criteria:**
  ...
  - Packaged Ark Code smoke covers session creation/resume, a multi-step read-only investigation without per-step clicks, natural-language follow-up, stop/cancel, inline edit approval and rejection, allowed verification command, final summary, close/reopen recovery, and switching to/from Ark Chat without cross-surface state or authority leakage.
...
UX-001/002 establish three responsive layout modes. Ark Chat and Ark Code are sibling primary destinations within each mode; the application remembers the selected surface and its local state when switching:
...
Within the Ark Code destination, the coding conversation and composer remain primary at every supported width. Session navigation and repository/diff/terminal/context views collapse from panes to drawers/sheets before they displace the transcript. Manual tool forms are never the responsive fallback.
...
11. Switch Ark Chat → Ark Code → select/bind Repository → start or resume a coding conversation → send a natural-language request → observe automatic investigation → review/reject/approve inline changes or commands → stop/steer/follow up → inspect verification and final summary → close/reopen and reconcile safely.
...
UX-003, FTR-005, CMP-001/002, CODE-006/007:
...
- Ark Code uses the same readable conversation hierarchy while adding typed progress, tool, command-output, diff, approval, compaction, recovery, and final-summary timeline items; supporting panes never replace the transcript/composer as the main interaction.
...
- Cover Ark Code as a conversation-driven agent rather than a tool dashboard: initial request and follow-up, automatic multi-step work, incremental progress, inline tool/diff/approval items, stop/steer/retry, context compaction, session recovery, and Ark Chat/Ark Code switching. A test that drives a hidden single-step developer harness is not release evidence.
...
- Ark Code satisfies the authoritative product and interaction contract in Phase 6.5: users operate it through persistent natural-language coding conversations with automatic controlled execution, inline tools/diffs/approvals, follow-ups, cancellation, recovery, and supporting panes—not through a manual tool or per-step dashboard—and its reviewed behavioral-parity checklist records any intentional Ark-specific differences from Claude Code/OpenAI Codex;
