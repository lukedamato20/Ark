...
- **Status: Blocked by genuine external dependency (2026-08-14).** This is a "define..." task by its own description, done deliberately ahead of any real consumer — no RAG/tool/web/agent feature exists yet for it to govern. Everything locally implementable is done; what remains is the same kind of engineering/product sign-off ADR 0001 (FND-002) needed before being marked Complete, and self-approving a policy every future tool-calling feature is bound by would be exactly the "this agent cannot self-approve" situation FND-002 already established a precedent for.
  ...
  - Unblock by reviewing `docs/adr/0002-tool-capability-and-prompt-injection-policy.md` and recording approval in its own "Approval" section, the same pattern ADR 0001 used.
