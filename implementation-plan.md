...
- **Status: Partial — projects and personas are both complete and real; files/context policy and instructions/untrusted-content separation remain out of scope (2026-08-15, personas added; projects originally landed 2026-08-15 earlier the same day).** Projects were investigated and built first (see below, unchanged from the original pass); personas were deferred at the time as a large, largely-independent second capability rather than risking a half-finished mega-change, then picked up as this session's next unit of work once Phase 5 (FTR-001–010) was fully complete. Investigated the actual persona state before writing anything: `grep -ri persona` still hit only docs — genuinely greenfield, no table, no precedence tier, no UI.
  ...
  - **Not done, honestly — this remains Partial, not Complete:**
    - **Precedence documentation/visibility (criterion 1) is still only half-done.** The chain is now genuinely five-tier and every generation records exactly which tier (including persona + its version number) supplied each value in provenance — but there is still no user-facing explainer anywhere in the UI surfacing "application → project → persona → conversation → user" as a documented, visible concept. "Application" (a workspace-wide default tier) still doesn't exist as a concept at all.
    - **Instructions/untrusted-content separation (criterion 4) is unaddressed, and structurally can't be meaningfully attempted yet**: `ProviderChatRequest`/`ChatMessage` are unchanged from FTR-004's shape (`{ role, content }`), with no separate channel for injected instructions vs. retrieved content. There is still no file/attachment/retrieval feature anywhere in Ark producing untrusted content to separate *from* — building a separation mechanism now would be speculative scope with nothing real to protect against yet.
    - **Files/context policy** (part of the Description) — no code anywhere; out of scope for this pass, tracked as remaining project scope, most naturally picked up alongside CMP-001 (attachments).
    - **Personas are not exportable independently of a conversation** — FTR-008's workspace export/import (built before personas existed) has no awareness of personas at all; a persona a conversation references isn't included in or restored by a workspace export. A conversation's `personaId` round-trips as a plain field, but the referenced persona itself does not travel with it. This is a real, named gap between this task's criterion 2 ("exportable") and what actually ships.
    - **No dedicated per-persona conversation filter** — unlike projects (which get a `projectId` filter on `ConversationListRequest`, unused by any current UI), personas have no equivalent; deliberately not added since nothing in this pass's scope needed it.
