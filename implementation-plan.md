...
- **Status: Partially complete (2026-08-17).** Ark now has a first-class `code` ActiveView reachable from the main sidebar, a lazy-loaded `features/code` module, its own normalized domain store, persistent Project-owned session creation/list/detail flows, and a production read-only Repository inspector for CODE-004's map/search/read/Git operations. Migration `0018_ark_code_sessions.sql` establishes ADR-0003's authoritative sessions, immutable parent/child run attempts, hard budget snapshots, steps, exact tool invocations/approval hashes, bounded observations, sequenced versioned events, and operation/run-scoped idempotency receipts. Typed Tauri commands and `ArkClient` methods can create idempotent sessions and queued runs; run creation snapshots a replacement-sensitive but content-stable Repository identity and rejects disabled providers, unavailable models, and tool-unsupported models. Project ownership cascades safely, request-key reuse conflicts on changed content, and session/run/event DTOs pass the shared contract. Remaining before completion: connect the provider-driven read-only agent loop, persist/render real step/tool/observation cards, implement ADR-0003 cancellation/startup recovery and child-run resume decisions, add schema-versioned run event notifications/reconciliation, and then add diff/approval cards when CODE-005 lands. Current validation: 501 Rust tests pass (1 intentionally ignored), strict format/clippy, 77 DTO contracts, lint/typecheck, 67 frontend tests, production frontend build, architecture/design/security/support/supply-chain gates, and packaged MSI/NSIS build all pass.
